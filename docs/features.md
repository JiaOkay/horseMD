...
> **双击放大**：双击编辑区里的图片会在灯箱里放大查看（点背景 / ✕ / Esc 关闭）。`Editor.jsx` 在 `view.dom` 上监听无修饰键的 `dblclick`，命中 `<img>` 就用其 `currentSrc` 打开 `.hm-image-lightbox` 覆盖层。**纯显示**，不改文档。用 `dblclick`（不是 `click`）是为了把**单击留给 Crepe 原生的图片交互**（选中 + 编辑图片说明/caption）—— 之前用 `click` 会截胡单击,导致 caption 编辑失效、只剩选中线框。
