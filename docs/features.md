...
> 重命名时输入框默认选中**文件名（不含扩展名）**，和新建输入框一致（`Sidebar.jsx` 的 rename input `onFocus` 里 `setSelectionRange(0, dotIndex)`）。
...
**实现**（`App.jsx`）：`isPlainTextDoc(tab)`（路径存在且不匹配 `MD_DOC_RE`）决定每个标签的编辑器；富文本标签常驻挂载，纯文本标签只在激活时渲染（避免后台重型 txt 拖慢应用）。详见 [implementation-notes.md](./implementation-notes.md)。
